...
import { ManifestBooking } from './PendingManifest';
...
function matches(Bookings: ManifestBookingList, term: string, pipe: PipeTransform) {
	...
	return (
		Bookings.Consignor_name.toLowerCase().includes(term.toLowerCase()) ||
		Bookings.DeliveryCity.toLowerCase().includes(term.toLowerCase())||
...
export class ManifestService {
    ...
    // Fetch the data
		...
		const apiUrl = `http://127.0.0.1:8000/BlindManifest/BlindManifestList/?branch_id=${branch_id}`;
		...
		return this.http.get(apiUrl).pipe(
		  switchMap((data: any) => {
			...
			const transformedData = data.map((item: any, index: number) => ({
			  ...
			  Contact: item.Client_Id__Client_Phone,
			  PickupTruck: item.Pickup_Truck,
 			  Total_Consignment: item.Total_Consignment,
